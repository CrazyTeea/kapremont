      ...
      <div class="row">
        <div class="col-3">
          <b-dropdown v-can:mgsu,root right text="статус эксперта МОН" size="xs" variant="info" class="m-2">
            <b-dropdown-item v-if="items.id_founder ===1" @click="setStatus(2)" variant="success">Рекомендуется к
              согласованию
            </b-dropdown-item>
            <b-dropdown-item v-if="items.id_founder ===1" @click="setStatus(3)" variant="danger">Не рекомендуется к
              согласованию
            </b-dropdown-item>
            <b-dropdown-item v-if="items.id_founder ===1" @click="setStatus(4)" variant="warning">На доработку
            </b-dropdown-item>
            <b-dropdown-item v-if="items.type || items.id_founder !==1" @click="setStatus(5)" variant="info">В
              реализации
            </b-dropdown-item>
          </b-dropdown>
        </div>
        <div v-if="items.id_founder===1" class="col-4">
          <b-dropdown v-can:dep,root right text="статус ДЭП" variant="info" class="m-2">
            <b-dropdown-item :href="`/api/set-status/approved/dep/${obj_id}`" @click="actionHendler" variant="success">
              Рассмотрено ДЭП
            </b-dropdown-item>
            <b-dropdown-item :href="`/api/set-status/rejected/dep/${obj_id}`" @click="actionHendler" variant="warning">
              Резерв
            </b-dropdown-item>
          </b-dropdown>
        </div>
                ...
                <b-tr v-for="(item,index) in svedenia2" :key="index" @change="sendData(item)"
                  ...
                  <b-td v-if="!(item.hasOwnProperty('button') && item.button)">
                    ...
                    <b-form-select @change="sendData(item)" :plain="true" :options="months"
                                   ...
                                   v-can:user,faiv_user,root v-else-if="item.canDelete && item.step > 7"
                  ...
                  <b-td style="max-width: 10px" v-if="!(item.hasOwnProperty('button') && item.button)">
                    <b-form-input style="font-size: 10px" v-can:user,faiv_user,root type="number" step=".01"
                  ...
                  <b-td v-if="!(item.hasOwnProperty('button') && item.button)">
                    <b-form-textarea style="font-size: 10px" aria-invalid="true" size="sm" type="area"
