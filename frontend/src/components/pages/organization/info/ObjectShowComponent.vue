<template>
  <div>
    ...
    <div>
      <a v-if="canChange" :href="`/program/object/update/${this.items.id}`"
         class="btn btn-primary">Редактировать </a>
      <b-button v-if="planExist()" v-can:root @click="tableToExel()">Выгрузить план график</b-button>
      <a
          v-if="this.real"
    ...
    <div v-if="this.items.status == 5">
      <div class="row">
        <div class="col-2">
          <h5>Текущий статус эксперта МОН: <label :class="`text-${status.variant}`">{{ status.label }}</label>
          </h5>
        </div>
        <div v-if="items.id_founder===1" class="col-3">
          <h5>Текущий статус ДЭП: <label :class="`text-${dep_status.color}`">{{ dep_status.label }}</label>
          </h5>
        </div>
        <!--<div class="col-2">
      ...
      <div class="row">
        <div class="col-3">
          <b-dropdown v-can:mgsu,root right text="статус эксперта МОН" size="xs" variant="info" class="m-2">
            <b-dropdown-item v-if="items.id_founder ===1" @click="setStatus(2)" variant="success">
              Рекомендуется к
              согласованию
            </b-dropdown-item>
            <b-dropdown-item v-if="items.id_founder ===1" @click="setStatus(3)" variant="danger">Не
              рекомендуется к
              согласованию
            </b-dropdown-item>
            <b-dropdown-item v-if="items.id_founder ===1" @click="setStatus(4)" variant="warning">На
              доработку
            </b-dropdown-item>
            <b-dropdown-item v-if="items.type || items.id_founder !==1" @click="setStatus(5)"
                             variant="info">В
              реализации
            </b-dropdown-item>
          </b-dropdown>
        ...
        <div v-if="items.id_founder===1" class="col-4">
          <b-dropdown v-can:dep,root right text="статус ДЭП" variant="info" class="m-2">
            <b-dropdown-item :href="`/api/set-status/approved/dep/${obj_id}`" @click="actionHendler"
                             variant="success">
              Рассмотрено ДЭП
            </b-dropdown-item>
            <b-dropdown-item :href="`/api/set-status/rejected/dep/${obj_id}`" @click="actionHendler"
                             variant="warning">
              Резерв
            </b-dropdown-item>
          </b-dropdown>
        ...
        <div v-if="items.id_founder===1" class="col-4">
          <b-dropdown v-can:root,dep right class="m-2" :text="realStatusType[realStatus].label"
                      :variant="realStatusType[realStatus].variant">
            <b-dropdown-item v-for="(item,index) in realStatusType" @click="changeRealStatus(index)"
                             :key="index" :variant="item.variant" :value="index">{{ item.label }}
            </b-dropdown-item>
          </b-dropdown>
        </div>
      </div>
    ...
    <div v-if="this.items.status == 5">
            ...
            <div class="wraper-for-chart" style="overflow-x: scroll">
              <div class="chart-wrapper">
                ...
                <ul class="labels-for-chart">
                  ...
                  <li>Выполнение обследования, подготовка и утверждение дефектного акта (дефектной
                    ведомости)
                  </li>
                  <li>Утверждение задания на проектирование</li>
                  <li>Проведение тендера и заключение договора на подготовку проектно-сметной
                    документации
                  </li>
                  <li>Подготовка проектно-сметной документации</li>
                  <li>Прохождение экспертизы проектно-сметной документации</li>
                  <li>Проведение тендера и заключение договора на выполнение строительно-монтажных
                    работ
                  </li>
      ...
      <b-card no-body class="mb-1">
        ...
        <b-collapse id="accordion-plan2" accordion="my-accordion2" role="tabpanel">
          <b-card-body>
            <b-table-simple id="exportPlan" class="mt-3" small sticky-header bordered
                            style=" font-size: 10px; min-height: 1000px">
              <b-thead>
                <b-tr>
                  <b-th></b-th>
                  <b-th>Этап</b-th>
              ...
              <b-tbody>
                <b-tr v-for="(item,index) in svedenia2" :key="index" @change="sendData(item)"
                      :variant="!isFloat(item.step) ? 'success' : ''">
                  <b-td v-if="!(item.hasOwnProperty('button') && item.button)">
                    {{ getMonth(item.step) }}
                  </b-td>
                  <b-td v-if="!(item.hasOwnProperty('button') && item.button)">
                    <b-form-input style="font-size: 10px" v-can:user,faiv_user,root
                                  v-if="item.canDelete && item.step < 7" v-model="item.step_name"/>
                    <b-form-select @change="sendData(item)" :plain="true" :options="months"
                                   style="font-size: 10px;-webkit-appearance: none;-moz-appearance: none;appearance: none;"
                                   v-can:user,faiv_user,root
                                   v-else-if="item.canDelete && item.step > 7"
                                   v-model="item.step_name"/>
                    <span v-else>{{ item.step_name }}</span>
                  </b-td>
                  <b-td v-if="!(item.hasOwnProperty('button') && item.button)">
                    <b-form-input style="font-size: 10px" v-can:user,faiv_user,root type="date"
                                  v-model="svedenia2[index].date_event_start"/>
                    <span v-can:mgsu,dep,dku,dku_user>{{ item.date_event_start }}</span>
                  </b-td>
                  <b-td v-if="!(item.hasOwnProperty('button') && item.button)">
                    <b-form-input style="font-size: 10px" v-can:user,faiv_user,root type="date"
                                  v-model="svedenia2[index].date_event_end"/>
                    <span v-can:mgsu,dep,dku,dku_user>{{ item.date_event_end }}</span>
                  </b-td>
                  <b-td style="max-width: 10px"
                        v-if="!(item.hasOwnProperty('button') && item.button)">
                    <b-form-input style="font-size: 10px" v-can:user,faiv_user,root type="number"
                                  step=".01"
                                  v-model="svedenia2[index].cost_real"/>
                    <span v-can:mgsu,dep,dku,dku_user>{{ item.cost_real }}</span>
                  </b-td>
                  <b-td v-if="!(item.hasOwnProperty('button') && item.button)">
                    <b-form-input style="font-size: 10px" v-can:user,faiv_user,root type="number"
                                  step=".01"
                                  v-model="svedenia2[index].sum_bud_fin"/>
                    <span v-can:mgsu,dep,dku,dku_user>{{ item.sum_bud_fin }}</span>
                  </b-td>
                  <b-td v-if="!(item.hasOwnProperty('button') && item.button)">
                    <b-form-input style="font-size: 10px" v-can:user,faiv_user,root type="number"
                                  step=".01"
                                  v-model="svedenia2[index].fin_vnebud_ist"/>
                    <span v-can:mgsu,dep,dku,dku_user>{{ item.fin_vnebud_ist }}</span>
                  </b-td>
                  <b-td v-if="!(item.hasOwnProperty('button') && item.button)">
                    <b-form-checkbox style="font-size: 10px" v-can:user,faiv_user,root
                                     v-model="svedenia2[index].done"/>
                  ...
                  <b-td style="max-width: 50px"
                    ...
                    <div v-can:user,faiv_user,root v-if="sved2Doc.includes(item.step + 1)"
                         class="fileInput">
                      <div
                    ...
                    <div v-if="svedenia2[index].file">
                      <a :href="`/program/event/download/${item.id}`">{{
                          svedenia2[index].file.name
                        }}</a>
                    </div>
                    <a v-if="svedenia2[index].access_document && svedenia2[index].access_document != 'null'"
                       :href=svedenia2[index].access_document
                       v-can:mgsu,dep,dku,dku_user>{{ svedenia2[index].access_document }}</a>
                  ...
                  <b-td style="max-width: 100px"
                        v-if="!(item.hasOwnProperty('button') && item.button)">
                    <b-form-input style="font-size: 10px" v-can:user,root,faiv_user
                                  v-model="svedenia2[index].comment"/>
                    <span v-can:mgsu,dep,dku,dku_user>{{ item.comment }}</span>
                  </b-td>
                  <b-td v-if="!(item.hasOwnProperty('button') && item.button)">
                    <b-form-checkbox style="font-size: 10px" v-can:root,mgsu,dep
                                     v-model="svedenia2[index].doneExpert"/>
                    <span v-can:user>
                                            {{ item.doneExpert ? 'Да' : 'Нет' }}
                                        </span>
                  </b-td>
                  <b-td v-if="!(item.hasOwnProperty('button') && item.button)">
                    <b-form-textarea style="font-size: 10px" aria-invalid="true" size="sm"
                                     type="area"
                                     v-can:root,mgsu,dep v-model="svedenia2[index].commentExpert"/>
                    <span v-can:user>
                                            {{ item.commentExpert }}
                                        </span>
                  </b-td>
                  <div v-can:user,faiv_user,root v-else>
                    <b-td>
                      <b-button size="sm" variant="danger" @click="deleteRow(index)">Удалить
                      </b-button>
                ...
                <div v-if="docs.length">
                  <table class="table table-bordered">
                    ...
                    <tr v-for="(doc, index) in docs" :key="index">
                      <td>{{ doc.label }}</td>
                      <td>
                        <a :href="`/program/object/download-doc/${obj_id}?id=${doc.id}`">
                          {{ doc.name }}.pdf
                        </a>
                      </td>
                    </tr>
                  </table>
                  <a class="btn btn-danger" target="_blank" :href="`/program/object/zip/${obj_id}`">Скачать
                    архивом</a>
                </div>
                <div v-else>
    ...
    <div v-else>
      ...
      <b-card v-if="checkFOIV()" no-body class="mb-1">
        <b-card-header header-tag="header" class="p-1" role="tab">
                    <span> <b-icon icon="document-text" scale="1.5"
                                   class="mr-2 ml-1"></b-icon>Загруженные документы</span>
        </b-card-header>
        <b-card-body>
          <div v-if="docs.length">
            <table class="table table-bordered">
...
export default {
  ...
  data() {
    return {
      ...
      sved2Href: [1.2, 1.3, 4.3, 4.2, 7.2, 7.3, 7.41, 8.01, 8.02, 8.03, 8.04, 8.05, 8.06, 8.07, 8.08, 8.09],
      sved2Doc: [1.1, 1.4, 2, 3, 1.3, 4.3, 4.1, 4.4, 5, 6, 7.1, 7.3, 7.4, 8, 8.01, 8.02, 8.03, 8.04, 8.05, 8.06, 8.07, 8.08],
      firstLoad: true
    };
  ...
  methods: {
    planExist() {
      return document.getElementById('exportPlan') !== null;
    },
    async run() {
