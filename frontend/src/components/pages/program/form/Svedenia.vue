  ...
  <div class="hidden-val-for-table">
    ...
    >
      ...
      <template v-slot:cell(final_date)="row">
        <b-form-input
            type="date"
            v-model="sved.final_date[row.index]"
      ...
      <template v-slot:cell(rc)="row">
        <b-form-input
            ...
            type="number"
            @change="
                        val => setFloat(val, row.index, 'realization_cost')
                    "
...
import {BFormInput, BFormSelect, BTable} from "bootstrap-vue";
  ...
  computed: {
    ...
    rc_full() {
      ...
      for (let key in this.sved.realization_cost) {
        sum +=
            parseFloat(this.sved.realization_cost[key]).toFixed(3) *
            1 || 0;
      }
